...
const SignUpScreen = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const userResponse = await axios.post("http://localhost:5001/add-user", {
                name: name,
                email: email,
                campus_id: campusId,
                school_id: id,
            ...
            if (userResponse.data.message) {
                alert(userResponse.data.message); // Success message

                // Step 2: Send OTP after successful signup
                const otpResponse = await axios.post("http://localhost:5001/send-otp", { email });

                if (otpResponse.data.message) {
                    // Redirect to the LoginAuth screen with the email passed as state
